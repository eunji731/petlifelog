...
public class DashboardReport extends BaseTimeEntity {
    ...
    private String monthlyHighlightsJson; // [{title, date, reason}]

    @Column(name = "monthly_tags_json", columnDefinition = "TEXT")
    private String monthlyTagsJson; // ["산책", "카페"]

    // ── 성격 인사이트 (AI 판단) ───────────────────────────────────────────────
    @Column(name = "personality_type", length = 30)
    private String personalityType; // NATURE_LOVER | SOCIAL | HOMEBODY | ADVENTURER | ROUTINE_KEEPER
    ...
    private String activityTrend; // UP | STABLE | DOWN | UNKNOWN

    @Column(name = "activity_level", length = 10)
    private String activityLevel; // GREAT | NORMAL | WATCH | WARNING | UNKNOWN

    @Column(name = "activity_confidence", length = 10)
    private String activityConfidence; // HIGH | LOW | NONE
    ...
    private String locationVerdict; // VARIED | FOCUSED | ROUTINE | LOW_DATA
