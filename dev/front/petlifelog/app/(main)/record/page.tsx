    ...
    <div className="flex-1 flex flex-col min-h-0 bg-surface-green/30 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6 lg:p-10">
        <div className="max-w-5xl mx-auto space-y-8">
          {allLogs.length > 0 ? (
            allLogs.map((log) => (
              <div key={log.id} className="bg-white rounded-[40px] border border-border shadow-sm overflow-hidden group hover:shadow-xl transition-all duration-500">
                <div className="flex flex-col lg:flex-row">
                  {/* Left: Summary Hero */}
                  <div className="lg:w-1/3 relative h-64 lg:h-auto overflow-hidden">
                    <Image src={log.representativePhotoPath || '/dog-profile.png'} alt="Daily" fill className="object-cover group-hover:scale-110 transition-transform duration-1000" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                    <div className="absolute top-6 left-6 px-3 py-1.5 bg-white/90 backdrop-blur-md rounded-full text-[10px] font-black text-main-green shadow-lg">
                      {log.dateKey}
                    </div>
                          ...
                          {log.moments.slice(0, 2).map(moment => (
                            <div key={moment.id} className="flex items-center gap-3 p-3 bg-surface-green/50 rounded-2xl border border-main-green/5">
                              <div className="relative w-10 h-10 rounded-xl overflow-hidden shrink-0 shadow-sm">
                                <Image src={moment.photos[0]?.path || '/dog-profile.png'} alt="Moment" fill className="object-cover" />
                              </div>
